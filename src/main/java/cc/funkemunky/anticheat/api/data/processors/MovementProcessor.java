...
public class MovementProcessor {
    private boolean isLagging, clientOnGround, serverOnGround, fullyInAir, inAir, hasJumped, inLiquid, blocksOnTop, pistonsNear, onHalfBlock,
            onClimbable, onIce, collidesHorizontally, inWeb, onSlimeBefore, onSoulSand, isRiptiding, halfBlocksAround, isNearGround, wasAbleToFly,
            wallBlocksClose, climablesClose;
    @Setter private boolean fallCancel;
    ...
    private float deltaY, yawDelta, pitchDelta, lastYawDelta, lastPitchDelta, lastDeltaY, deltaXZ, distanceToGround, serverYVelocity, lastServerYVelocity, serverYAcceleration, clientYAcceleration, lastClientYAcceleration, lastServerYAcceleration, jumpVelocity, cinematicYawDelta, cinematicPitchDelta, lastCinematicPitchDelta, lastCinematicYawDelta, realFallDistance, slimeDistance, lastFallDistance;
    ...
    private List<BoundingBox> boxes = new ArrayList<>(), closeBoxes = new ArrayList<>();
    private long lastTimeStamp;

    public void update(PlayerData data, WrappedInFlyingPacket packet) {
        val player = packet.getPlayer();
        val timeStamp = System.currentTimeMillis();
        Iris.getInstance().getProfiler().start("MovementProcessor:update");
        ...
        if (packet.isPos()) {
            ...
            List<BoundingBox> closeBox = closeBoxes = Atlas.getInstance().getBlockBoxManager().getBlockBox().getCollidingBoxes(player.getWorld(), data.getBoundingBox().grow(0.25f, 0.25f, 0.25f));


            CollisionAssessment assessment = new CollisionAssessment(data.getBoundingBox(), data);
            CollisionAssessment closeAssessment = new CollisionAssessment(data.getBoundingBox(), data);
            ...
            player.getNearbyEntities(1, 1, 1).stream().filter(entity -> entity instanceof Vehicle || entity.getType().name().toLowerCase().contains("shulker")).forEach(entity -> closeAssessment.assessBox(ReflectionsUtil.toBoundingBox(ReflectionsUtil.getBoundingBox(entity)), player.getWorld(), true));
            ...
            closeBox.forEach(cbb -> closeAssessment.assessBox(cbb, player.getWorld(), false));
            ...
            wallBlocksClose = closeAssessment.getMaterialsCollided().stream().anyMatch(material -> material.toString().contains("COBBLE_WALL") || material.toString().contains("FENCE"));
            climablesClose = closeAssessment.getMaterialsCollided().stream().anyMatch(material -> material.toString().contains("LADDER") || material.toString().contains("VINE"));
            ...
            if (wasAbleToFly && !player.getAllowFlight()) {
                data.getLastAbleToFly().reset();
            }
            wasAbleToFly = player.getAllowFlight();
            ...
            if (Iris.getInstance().getCurrentTicks() % 4 == 0) {
            ...
            if (packet.isGround() || inLiquid || inWeb || data.getPlayer().isInsideVehicle()) {
                realFallDistance = slimeDistance = 0;
            } else if (deltaY < 0) {
                slimeDistance = (realFallDistance -= deltaY) * 1.5f;
            }
        }
        ...
        Iris.getInstance().getProfiler().stop("MovementProcessor:update");
    }

    public boolean isNearGround(PlayerData data, float amount) {
        Iris.getInstance().getProfiler().start("MovementProcessor:isNearGround");
    ...
    public boolean isOnGround(PlayerData data, float amount) {
        Iris.getInstance().getProfiler().start("MovementProcessor:isOnGround");
        ...
        boolean near = boxes.stream()
                .anyMatch(box2 -> data.getBoundingBox().grow(1E-6f, 0.5f,1E-6f).intersectsWithBox(box2) && box.collides(box2) && getTo().getY() + 0.1f >= box2.getMaximum().getY() && !BlockUtils.isSolid(BlockUtils.getBlock(box2.getMinimum().toLocation(data.getPlayer().getWorld()).clone().add(0,1,0)))
                        && box2.collidesVertically(box));
        Iris.getInstance().getProfiler().stop("MovementProcessor:isOnGround");
        return near;
    }
